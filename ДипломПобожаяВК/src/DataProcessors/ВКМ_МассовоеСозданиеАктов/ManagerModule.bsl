...
Функция  СоздатьАктыНаСервере(НачалоПериода, КонецПериода) Экспорт 
	...
	Для Каждого СтрокаТЗ Из ТабЗнач Цикл 
		...
		МассивДляТЧ.Добавить(Новый Структура("Договор, Реализация", СтрокаТЗ.Договор, СтрокаТЗ.Реализация));
...
Функция СоздатьНовыйДокРеализации(СтрокаТЗ, Дата)
	...
	ПроверкаНеПрошла = НовыйДок.ПроверитьЗаполнение(); 
	Если НЕ ПроверкаНеПрошла Тогда
